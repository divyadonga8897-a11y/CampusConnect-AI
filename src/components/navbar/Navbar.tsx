  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-50 w-full bg-[#0B0F19]/60 backdrop-blur-md border-b border-slate-900/40 py-2 sm:py-2.5 px-4 sm:px-6 lg:px-8 transition-all duration-200">
        <header
          className={`mx-auto max-w-6xl w-full rounded-full border border-slate-800/70 bg-[#0F172A]/85 transition-all duration-300 ${
            scrolled 
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`text-base font-medium tracking-tight transition-all duration-300 cursor-pointer px-3.5 py-2 select-none relative group ${
                      active
                        ? "text-white"
                  ...
                  >
                    ...
                    <span className={`absolute bottom-0.5 left-3.5 right-3.5 h-[2.5px] bg-blue-500 rounded-full transition-all duration-350 origin-left ${
